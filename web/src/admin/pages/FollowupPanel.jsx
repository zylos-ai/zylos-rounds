...
const DICT = {
  zh: {
    title: '补充与跟进',
    sub: '会后补充或更新的信息——下一期会自动带给 AI，已拍板的会在汇总里收口，不再重复追问。',
    empty: '还没有补充。开完会把拍板结论或新信息填在下面，AI 下次就知道了。',
  ...
  en: {
    title: 'Follow-ups',
    sub: 'Info appended or updated after a round — carried into the next cycle for the AI; settled items are closed out in the digest, not re-probed.',
    empty: 'No follow-ups yet. After a meeting, note the decision or new info below and the AI will know next time.',
...
/**
 * Follow-up panel for a task's detail page. Lists the task's recent follow-ups
 * and lets the owner append a new one (private by default, or team-shared).
 * A follow-up is plain text — the general carry-forward container the daily
 * decision write-back dissolved into.
 */
export default function FollowupPanel({ taskId }) {
  ...
  const load = useCallback(async () => {
    try {
      const r = await api(`api/followups?task_id=${taskId}`);
    ...
    <Card>
      ...
      <CardContent className="space-y-4 pt-1">
        ...
        {items.length ? (
          ...
          <p className="rounded-md border border-dashed border-border px-3.5 py-4 text-center text-sm text-muted-foreground">{T.empty}</p>
        )}
